...
import { Users, Package, CheckCircle, AlertTriangle, MessageSquare, Shield, QrCode } from "lucide-react";
...
export const AdminPage = () => {
  ...
  const startQRVerification = (claim: typeof pendingClaims[0]) => {
    setSelectedClaim(claim);
    setIsQRVerificationOpen(true);
  };
    ...
    if (selectedClaim) {
      if (success) {
        ...
        toast({
          ...
          description: `QR verification failed for ${selectedClaim.itemName}. Additional proof required.`,
          variant: "destructive"
        });
      }
    }
  ...
  const approveClaim = (claimId: string) => {
    ...
    toast({
      ...
      description: `The claim for ${claim?.itemName} has been approved and users will be notified.`,
    });
  };
  ...
  const rejectClaim = (claimId: string) => {
    ...
    toast({
      ...
      description: `The claim for ${claim?.itemName} has been rejected with feedback sent to the user.`,
    });
  };

  const contactUser = (userName: string, itemName?: string) => {
    setChatRecipient({ type: "user", name: userName, itemName });
    setIsChatOpen(true);
  };
  ...
  return (
    <>
                      ...
                      <TableRow key={claim.id}>
                        ...
                        <TableCell>
                          <div className="flex space-x-2">
                            <Button 
                              size="sm"
                              onClick={() => startQRVerification(claim)}
                              variant="default"
                            >
                            ...
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => contactUser(claim.claimant, claim.itemName)}
                            >
                      ...
                      <TableRow key={report.id}>
                        ...
                        <TableCell>
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => contactUser(report.user, report.item)}
                          >
      ...
      <ChatBox
        ...
        itemName={chatRecipient.itemName}
      />
      ...
      {selectedClaim && (
        <QRVerification
          ...
          claimantName={selectedClaim.claimant}
